...
public:
    ...
    /// @brief 设置相机自身的世界坐标
    HAZEL_API void setPosition(const glm::vec3& position)
    {
        m_position = position;
        recalculateViewMartix();
    }
    /// @brief 返回相机当前基于z轴的逆时针旋转角度
    HAZEL_API float getRotation() const
    {
        return m_rotation;
    }
    /// @brief 设置2d相机基于z轴逆时针的旋转
    /// @param rotation 旋转角度
    HAZEL_API void setRotation(float rotation)
    {
        m_rotation = rotation;
        recalculateViewMartix();
    }
    ...
    /// @brief 返回正交相机的正交投影矩阵
    HAZEL_API const glm::mat4& getProjectionMatrix() const
    {
        return m_projection_matrix;
    }
    /// @brief 返回p x v 矩阵缓存(opengl列向量视角)
    HAZEL_API const glm::mat4& getProjectionViewMatrix() const
    {
...
private:
    ...
    HAZEL_API void recalculateViewMartix();
