...
{
    auto& io = ImGui::GetIO();
    io.AddMousePosEvent(event.getPosX(), event.getPosY());
    return false;
}
...
bool ImGuiLayer::onKeyTypedEvent(KeyTypedEvent& event)
{
    // TODO: 支持中文输入?
    auto& io = ImGui::GetIO();
    auto key_code = (unsigned int)event.getKeyCode();
...
{
    auto& io = ImGui::GetIO();
    io.DisplaySize = ImVec2{(float)event.getResizeWidth(), (float)event.getResizeHeight()};
    io.DisplayFramebufferScale = ImVec2{1.0f, 1.0f};
    glViewport(0, 0, event.getResizeWidth(), event.getResizeHeight());
    return false;
}
